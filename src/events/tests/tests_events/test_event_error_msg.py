...
def test_event_attendee_negative(gestionnaire, support, commercial, commercial_client):
    ...
    assert str(response.data["non_field_errors"][0]) == MSG_ERR_EVENT['ERR_ATT']
...
def test_event_not_sup(gestionnaire, commercial, commercial_client):
    ...
    response = commercial_client.post(f"/customers/{customer.id}/contracts/{contract.id}/events/", data=data)
    assert response.status_code == status.HTTP_400_BAD_REQUEST, response.content
    assert str(response.data["non_field_errors"][0]) == MSG_ERR_EVENT['ERR_USER']


@pytest.mark.django_db
def test_event_date_begin(gestionnaire, commercial, commercial_client):
    """Create contract ok, role GES"""
    customer = Customer(enterprise_name='BELGACOM',
                        client_name='moi',
                        information='Lorem ipsum dolor sit amet, consectetur adipiscing elit. Aenean ',
    ...
    response = commercial_client.post(f"/customers/{customer.id}/contracts/{contract.id}/events/", data=data)
    assert response.status_code == status.HTTP_400_BAD_REQUEST, response.content
    assert str(response.data["non_field_errors"][0]) == MSG_ERR_EVENT['ERR_DATE']


@pytest.mark.django_db
def test_event_hour_begin(gestionnaire, commercial, commercial_client):
    """Create contract ok, role GES"""
    customer = Customer(enterprise_name='BELGACOM',
                        client_name='moi',
                        information='Lorem ipsum dolor sit amet, consectetur adipiscing elit. Aenean ',
    ...
    response = commercial_client.post(f"/customers/{customer.id}/contracts/{contract.id}/events/", data=data)
    assert response.status_code == status.HTTP_400_BAD_REQUEST, response.content
    assert str(response.data["non_field_errors"][0]) == MSG_ERR_EVENT['ERR_HOUR']
